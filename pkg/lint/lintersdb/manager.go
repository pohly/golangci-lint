...
func (m Manager) GetAllSupportedLinterConfigs() []*linter.Config {
	...
	lcs := []*linter.Config{
		...
		linter.NewConfig(golinters.NewForbidigo(forbidigoCfg)).
			...
			// information is only needed when
			// forbidigoCfg.AnalyzeTypes is chosen by the
			// user. But we don't know that here in all
			// cases (sometimes config is not loaded), so
			// we have to assume that it is needed to be on
			// the safe side.
			WithLoadForGoAnalysis().
